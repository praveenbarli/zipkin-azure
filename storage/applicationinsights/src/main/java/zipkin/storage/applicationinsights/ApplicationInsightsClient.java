...
public class ApplicationInsightsClient {
  ...
  private final String AI_QUERYTOGETUNIQUESERVICENAMES = "let namespace = %s;"
      + "let getSpan = (customDimensions:dynamic){"
      + "    strcat(tostring(customDimensions['zipkin-span0']), tostring(customDimensions['zipkin-span1']), tostring(customDimensions['zipkin-span2']), tostring(customDimensions['zipkin-span3']), tostring(customDimensions['zipkin-span4']),"
      + "    tostring(customDimensions['zipkin-span5']), tostring(customDimensions['zipkin-span6']), tostring(customDimensions['zipkin-span7']))"
      + "};"
      + "let spansData = requests "
      + "| extend span =  getSpan(customDimensions)"
      + "| where (isnull(namespace) or isempty(tostring(namespace)) or customDimensions.['namespace'] == tostring(namespace)) and (isnotnull(span) and isnotempty(span))"
      + "| project span "
      + "| union ("
      + "dependencies"
      + "| extend span =  getSpan(customDimensions)"
  ...
  private final String AI_QUERYTOGETUNIQUESPANNAMES = "let namespace = %s;"
      ...
      + "| summarize by spanName = tostring(parsejson(span).name)"
      + "| union"
      + "("
      + "spans"
      + "| extend binAnns = parsejson(span).binaryAnnotations"
      + "| mvexpand binAnns"
  ...
  private final String AI_QUERYTOGETTRACES = "let namespace = %s;"
      ...
      + ""
      + "let spans =  requests"
      + "| extend span = parsejson(getSpan(customDimensions))"
      + "| where (isnull(namespace) or isempty(tostring(namespace)) or customDimensions.['namespace'] == tostring(namespace)) and (isnotnull(span) and isnotempty(span))  and ((minDuration == 'null' or tolong(span.duration) >= tolong(minDuration)) and (maxDuration == 'null' or tolong(span.duration) <= tolong(maxDuration))) and (tolower(spanName) == 'all'  or spanName == 'null' or (isnotnull(span.name)  and tostring(span.name) == spanName)) and tolong(span.timestamp) <= tolong(endtime) and tolong(span.timestamp) >= tolong(starttime)"
      + "| project span"
      + "| union    "
      + "( "
      + "dependencies"
      ...
      + "| summarize ann_set = makeset(iff(isnotnull(binAnnotations.key) and binAnnotations.key in (annotationList),tostring(binAnnotations.key),\" \")) , binAnn_KeyValue = makeset(iff(isnotnull(binAnnotations.key) and isnotnull(binAnnotations.value) and strcat(binAnnotations.key, binAnnotations.value) in (binAnnotationKeyValuePairs),strcat(binAnnotations.key, binAnnotations.value), \" \")) , spanList = makeset(span)  by tostring(span.traceIdHigh), tostring(span.traceId), tostring(span.id))"
      ...
      + ""
      + "let resultTraces =  requests"
      + "| extend span = parsejson(getSpan(customDimensions))"
      + "| where (isnull(namespace) or isempty(tostring(namespace)) or customDimensions.['namespace'] == tostring(namespace)) and (isnotnull(span) and isnotempty(span)) and (strcat(customDimensions.['traceIdHigh'], customDimensions.['traceId']) in (traceIds))"
      + "| project span"
      + "| union (  "
      + " dependencies"
      + "| extend span = parsejson(getSpan(customDimensions))"
  ...
  private final String AI_QUERYTOGETTRACE = "let namespace = %s;"
      ...
      + "};"
      + ""
      + "let traceSpans =  requests"
      + "| extend span = parsejson(getSpan(customDimensions))"
      + "| where (isnull(namespace) or isempty(tostring(namespace)) or customDimensions.['namespace'] == tostring(namespace))"
      + " and (isnotnull(span) and isnotempty(span))"
      + "and span.traceId == traceId and (strictTraceId == false or isempty(tostring(traceIdHigh)) or isnull(span.traceIdHigh) or span.traceIdHigh == '0L' or span.traceIdHigh == traceIdHigh)"
      + "| project span"
      + "| union ("
      + " dependencies"
      + "| extend span =  parsejson(getSpan(customDimensions))"
